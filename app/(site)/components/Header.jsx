...
const Header = ({pages}) => {
  ...
  return (
    <header className={`fixed flex justify-end top-0 w-full z-20 ${isNavOpen ? 'bg-white' : 'bg-slate-50		'} md:bg-slate-50 md:px-6`}>
      ...
      <nav className={`absolute top-10 left-0 drop-shadow-md fixed w-full flex flex-col bg-white ${isNavOpen ? 'h-min p-3' : 'h-0 p-0'} overflow-hidden transition md:static md:h-10 md:top-0 md:flex-row md:gap-5 md:justify-end md:bg-slate-50 md:drop-shadow-none md:max-w-5xl md:mx-auto`}>
          {pages.sort((a, b) => {
